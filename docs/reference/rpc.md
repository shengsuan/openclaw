...
Clawer integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Clawer owns lifecycle when `channels.signal.autoStart=true`.
...
- Clawer spawns `imsg rpc` as a child process (legacy iMessage integration).
